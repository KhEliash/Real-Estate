...
const StateCards = ({ cardItem }) => {
  ...
  return (
    <div className="card card-compact h-[]  bg-base-100 shadow-xl p-3 ">
      <figure>
        <img src={image} alt="card-img" className="w-full h-[300px]" />
      </figure>
      <div className="space-y-3 mt-2 ">
        <h2 className="card-title"> {estate_title}</h2>
        <div className="font-bold flex justify-end text-lg text-green-500">{price}</div>

        <div className="flex  justify-between">
          <p>#{segment_name}</p>
          <p className="font-bold">For: <span className="text-red-600">{status}</span></p>
        </div>
        <p className="flex items-center gap-2 text-gray-500"><FaLocationDot className="text-red-500"/> {location}</p>
